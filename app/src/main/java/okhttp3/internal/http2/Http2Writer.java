...
public final class Http2Writer implements Closeable {
    ...
    public final synchronized void connectionPreface() throws IOException {
        ...
        if (this.client) {
            ...
            if (logger2.isLoggable(Level.FINE)) {
                logger2.fine(Util.format(">> CONNECTION " + Http2.CONNECTION_PREFACE.mo34577e(), new Object[0]));
            }
            this.sink.mo34528T0(Http2.CONNECTION_PREFACE);
            this.sink.flush();
        }
    }
    ...
    public final void frameHeader(int streamId, int length, int type, int flags) throws IOException {
        ...
        if (!(length <= this.maxFrameSize)) {
            StringBuilder m24u = C0000a.m24u("FRAME_SIZE_ERROR length > ");
            m24u.append(this.maxFrameSize);
            m24u.append(": ");
            m24u.append(length);
            throw new IllegalArgumentException(m24u.toString().toString());
        }
        if (!((((int) 2147483648L) & streamId) == 0)) {
            throw new IllegalArgumentException(C0000a.m7d("reserved bit set: ", streamId).toString());
        }
    ...
    public final synchronized void headers(boolean outFinished, int streamId, @NotNull List<Header> headerBlock) throws IOException {
        ...
        long j2 = this.hpackBuffer.f68742c;
        ...
        this.sink.write(this.hpackBuffer, min);
        if (j2 > min) {
            writeContinuationFrames(streamId, j2 - min);
        }
    }
    ...
    public final synchronized void pushPromise(int streamId, int promisedStreamId, @NotNull List<Header> requestHeaders) throws IOException {
        ...
        long j2 = this.hpackBuffer.f68742c;
        int min = (int) Math.min(this.maxFrameSize - 4, j2);
        long j3 = min;
        frameHeader(streamId, min + 4, 5, j2 == j3 ? 4 : 0);
        this.sink.writeInt(promisedStreamId & Integer.MAX_VALUE);
        this.sink.write(this.hpackBuffer, j3);
        if (j2 > j3) {
            writeContinuationFrames(streamId, j2 - j3);
        }
    }
